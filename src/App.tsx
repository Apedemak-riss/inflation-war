...
import { Shield, Sword, ShoppingCart, Coins, ExternalLink, Hammer, Crown, Minus, Check, Users, RefreshCw, Trash2, Trophy, ArrowRightLeft, LogOut, Gavel, ClipboardCheck, AlertTriangle, Loader2, Edit2, Save, X, Tv, PawPrint } from 'lucide-react';
...
const getImageUrl = (name: string, type: ItemType, hero?: HeroType) => {
  ...
  if (type === 'pet') {
    // Format: Icon_HV_Hero_Pets_Name.png (spaces to underscores)
...
export function App() {
  ...
  const seedDatabase = async () => {
    ...
    await supabase.from('items').upsert(rows); // Changed to upsert to add pets to existing
    loadItems();
    setIsSeeding(false);
  };
  ...
  const handleFindLobby = async (e: React.FormEvent) => {
    ...
    setLobbyCode(code);
    let { data: lobby, error } = await supabase.from('lobbies').select('*').eq('code', code).single();
  ...
  const fetchTeams = async (lId: string) => {
    // Note: 'purchases' now includes 'equipped_hero'
    const { data: teams } = await supabase
        .from('teams')
        .select('*, players(id, name, purchases(item_id, equipped_hero))')
        .eq('lobby_id', lId)
        .order('name');
  ...
  const handleRefereeCheck = () => {
    // Referee update coming in next prompt
    setRefResult(0); 
  };
  ...
  const handleLeave = async () => { 
      if(confirm("Leave match?")) {
          // 1. Clear Session Instantly
          localStorage.clear(); 
          setPlayerId(null); 
          setTeamId(null);
          
          // 2. Force View Change Instantly
          setView('login');
          
          // 3. Send Cleanup Command to DB (Runs in background)
          if (playerId) await supabase.rpc('leave_team', { p_player_id: playerId }); 
      }
  };
  ...
  // --- GAME ACTIONS ---
  // FIXED: Force 0 for pets, otherwise calculate inflation
  const calcPrice = (base: number, id: string) => {
      const item = dbItems.find(i => i.id === id);
      if (item?.type === 'pet') return 0;
      return base + (teamPurchases.filter(p => p.item_id === id).length * 2);
  };
  ...
  const exportArmy = () => {
    ...
    const active = myPurchases.map(p => {
        const item = dbItems.find(i => i.id === p.item_id);
        return item ? { ...item, equipped_hero: p.equipped_hero } : null;
    }).filter(Boolean) as any[];

    // 2. Tally Troops, Spells, Sieges
    const counts: any = {}; 
    ...
    heroOrder.forEach(h => {
        ...
        // Find pet for this hero
        const pet = active.find(i => i.type === 'pet' && i.equipped_hero === h);
        // Find equipment for this hero
  ...
  const renderPlayerArmy = (p: any, isLarge = false) => {
      ...
      const active = myItems.map((x:any) => {
          ...
          return { ...item, equipped_hero: x.equipped_hero };
      }).filter(Boolean);

      const heroKeys = ['BK', 'AQ', 'GW', 'RC', 'MP'].filter(h => active.some((i:any) => i.hero === h || i.equipped_hero === h));
      ...
      return (
          <div className={`mt-2 space-y-2 ${isLarge ? 'space-y-6 mt-4' : ''}`}>
              ...
              {heroKeys.length > 0 && <div className={`flex flex-wrap ${isLarge ? 'gap-6' : 'gap-2'} justify-center lg:justify-start`}>
                  {heroKeys.map(h => (
                      <div key={h} className={`bg-slate-900 rounded ${isLarge ? 'p-3' : 'p-1'} border border-slate-700 flex items-center ${isLarge ? 'gap-3' : 'gap-1'}`}>
                          ...
                          {/* Pet (If assigned) */}
                          {active.filter((i:any) => i.type === 'pet' && i.equipped_hero === h).map((pet:any, idx:number) => (
              ...
              <div className={`flex flex-wrap ${isLarge ? 'gap-3' : 'gap-1'} justify-center lg:justify-start`}>
                  {unique.filter((i:any) => !i.hero && i.type !== 'pet').map((i:any) => (
  ...
  const renderArmySidebar = () => {
    // Re-fetch logic because we need equipped_hero
    const active = myPurchases.map(p => {
        ...
        return (
           <div key={i.id} className={`bg-slate-800 rounded-xl border p-3 relative transition-all duration-200 ease-out ${borderClass}`}>
                ...
                <div className="aspect-square bg-slate-900 rounded mb-2 relative overflow-hidden">
                   ...
                   {qty > 0 && i.type !== 'equipment' && i.type !== 'pet' && <div className="absolute top-0 right-0 bg-yellow-500 text-black px-1 rounded-bl font-bold shadow-md">x{qty}</div>}
                   {(isOwned || isOwnedPet) && <div className="absolute top-0 right-0 bg-green-500 text-black w-7 h-7 rounded-bl-lg flex items-center justify-center font-bold shadow-lg"><Check size={16}/></div>}
                </div>
        ...
        <div className="min-h-screen bg-slate-950 text-white flex items-center justify-center p-4">
          <div className="max-w-md w-full bg-slate-900 p-8 rounded-2xl border border-slate-800 text-center">
            ...
            <div className="mt-8 pt-4 border-t border-slate-800">
                <button onClick={seedDatabase} className="text-xs text-slate-500 hover:text-white flex items-center justify-center gap-2 mx-auto"><RefreshCw size={12}/> Refresh Database (Add Pets)</button>
            </div>
          </div>
        </div>
      );
  ...
  // (Referee, Streamer, Moderator views identical to previous except passed through render functions)
  // I will omit re-writing the full views here unless requested, but for safety in "App overwrite", I'll include the Game View structure changes.

  // ... [Assume Referee, Streamer, Moderator are same as previous logic] ...
  ...
  if (view === 'referee') return (
    <div className="min-h-screen bg-slate-950 text-white p-8 flex items-center justify-center">
      ...
      <div className="max-w-2xl w-full bg-slate-900 p-8 rounded-2xl border border-slate-800">
        ...
        <div className="space-y-4 mb-6">{refLinks.map((L, i) => <input key={i} value={L} onChange={e => { const n=[...refLinks]; n[i]=e.target.value; setRefLinks(n); }} className="w-full bg-black border border-slate-700 rounded p-3 text-sm font-mono" placeholder={`Paste Player ${i+1} Army Link`} />)}</div>
        ...
        {refResult !== null && (<div className={`text-center p-6 rounded-xl border-2 ${refResult > 100 ? 'bg-red-900/20 border-red-500' : 'bg-green-900/20 border-green-500'}`}><div className="text-sm text-slate-400 uppercase font-bold mb-2">Total Team Value</div><div className={`text-5xl font-black mb-2 ${refResult > 100 ? 'text-red-500' : 'text-green-500'}`}>{refResult} GOLD</div><div className="text-xl font-bold">{refResult > 100 ? <span className="flex items-center justify-center gap-2"><AlertTriangle/> ILLEGAL (OVER 100)</span> : <span className="flex items-center justify-center gap-2"><Check/> LEGAL</span>}</div></div>)}
  ...
  if (view === 'streamer') {
      ...
      <div className="min-h-screen bg-slate-950 text-white p-6 pb-20 overflow-hidden relative">
        <style>{`@keyframes marquee { 0% { transform: translateX(100%); } 100% { transform: translateX(-100%); } } .ticker-wrap { position: fixed; bottom: 0; left: 0; width: 100%; height: 50px; background: #000; overflow: hidden; white-space: nowrap; z-index: 100; border-top: 2px solid #334155; display: flex; align-items: center; } .ticker-content { display: inline-block; padding-left: 100%; animation: marquee 60s linear infinite; } .ticker-item { display: inline-block; padding: 0 2rem; font-family: monospace; font-size: 1.2rem; font-weight: bold; color: #fbbf24; }`}</style>
        {focusedPlayer && (<div className="fixed inset-0 bg-black/90 z-[200] flex items-center justify-center p-10 backdrop-blur-sm" onClick={() => setFocusedPlayer(null)}><div className="bg-slate-900 border-4 border-yellow-500 p-8 rounded-3xl shadow-2xl max-w-5xl w-full transform scale-110" onClick={e => e.stopPropagation()}><div className="flex justify-between items-center border-b-2 border-slate-700 pb-4 mb-6"><h2 className="text-6xl font-black text-white">{focusedPlayer.name}</h2><button onClick={() => setFocusedPlayer(null)} className="bg-red-600 hover:bg-red-500 text-white p-4 rounded-xl"><X size={32}/></button></div>{renderPlayerArmy(focusedPlayer, true)}</div></div>)}
        <header className="flex justify-between items-center mb-6 border-b border-slate-800 pb-4"><div className="flex items-center gap-3"><Tv className="text-purple-500 w-8 h-8"/><div><h1 className="text-2xl font-black">STREAM VIEW</h1><p className="text-slate-400 font-mono">LOBBY: {lobbyCode}</p></div></div><button onClick={() => setView('login')} className="bg-slate-800 hover:bg-slate-700 px-4 py-2 rounded font-bold">EXIT</button></header>
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-8 h-full">{lobbyTeams.map(team => (<div key={team.id} className="bg-slate-900 border-2 border-slate-800 rounded-2xl p-6 shadow-2xl h-fit"><div className="flex justify-between items-center mb-8 border-b border-slate-800 pb-6"><h2 className="text-5xl font-black text-white">{team.name}</h2><div className="bg-black px-6 py-3 rounded-lg border border-slate-700 font-mono text-yellow-500 text-4xl font-black">{team.budget} <Coins className="inline w-8 h-8"/></div></div><div className="flex flex-col gap-6">{[0, 1, 2].map(idx => { const p = team.players?.[idx]; if (!p) return <div key={idx} className="bg-black/20 rounded-xl p-4 border border-dashed border-slate-800 text-slate-700 text-center font-bold">EMPTY SLOT</div>; return (<div key={idx} onClick={() => setFocusedPlayer(p)} className="bg-black/40 rounded-xl p-6 border border-slate-700 hover:border-yellow-500 hover:bg-slate-800 transition-all cursor-zoom-in group"><div className="flex justify-between items-center border-b border-slate-600 pb-2 mb-2"><div className="font-black text-3xl text-slate-200 group-hover:text-yellow-400">{p.name}</div><div className="text-xs text-slate-500 uppercase font-bold bg-black px-2 py-1 rounded">Click to Zoom</div></div>{renderPlayerArmy(p, true)}</div>)})}</div></div>))}</div>
        <div className="ticker-wrap"><div className="ticker-content">{tickerItems.map((item:any, i:number) => (<div key={i} className={`ticker-item ${item.color}`}><span className="text-white text-sm mr-1 opacity-50">[{item.teamName}]</span>{item.name.toUpperCase()}: {item.price}g<span className="text-slate-500 ml-2 text-sm">▲</span></div>))}</div></div>
      </div>
  )}
  ...
  // --- GAME VIEW ---
  if (dbItems.length === 0) return (<div className="min-h-screen bg-slate-950 text-white flex flex-col items-center justify-center gap-4">{isSeeding ? <><Loader2 className="w-12 h-12 animate-spin text-blue-500"/><p>Seeding Database...</p></> : <><AlertTriangle className="w-12 h-12 text-yellow-500"/><p>Loading Game Data...</p><button onClick={()=>window.location.reload()} className="bg-blue-600 px-4 py-2 rounded">Retry</button></>}</div>);

  return (
    <div className="min-h-screen bg-slate-950 text-white flex flex-col lg:flex-row font-sans overflow-hidden">
      {/* PET MODAL */}
      {petModalItem && (
          <div className="fixed inset-0 bg-black/80 z-[100] flex items-center justify-center p-4 backdrop-blur-sm" onClick={() => setPetModalItem(null)}>
              <div className="bg-slate-900 border-2 border-slate-700 rounded-2xl p-6 w-full max-w-lg" onClick={e => e.stopPropagation()}>
                  <h3 className="text-2xl font-black mb-4 text-center">Assign {petModalItem.name} to:</h3>
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                      {['BK', 'AQ', 'GW', 'RC'].map(hero => (
                          <button key={hero} onClick={() => handleBuy(petModalItem, hero)} className="bg-slate-800 hover:bg-slate-700 border border-slate-600 rounded-xl p-4 flex flex-col items-center gap-2 transition-all">
                              <img src={`/${hero.toLowerCase()}.png`} className="w-16 h-16 rounded-full border-2 border-yellow-500 object-cover"/>
                              <span className="font-bold">{hero}</span>
                          </button>
                      ))}
                  </div>
                  <button onClick={() => setPetModalItem(null)} className="w-full mt-6 py-3 bg-red-900/50 text-red-200 rounded-lg hover:bg-red-800">Cancel</button>
              </div>
          </div>
      )}
        ...
        <main className="p-6 max-w-6xl mx-auto space-y-12">
            <div>
              ...
              {['BK','AQ','GW','RC','MP'].map(h => {
                ...
                return (<div key={h} className="mb-8 pl-4 border-l-2 border-slate-800"><h4 className="text-slate-400 font-bold mb-3 flex items-center gap-2"><img src={`/${h.toLowerCase()}.png`} className="w-8 h-8 rounded-full grayscale opacity-70 object-cover"/> {h} EQUIPMENT</h4>{renderGrid(hItems)}</div>)
              })}
            </div>
            {/* NEW PETS SECTION */}
